...
export function AdminFileActions() {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      // Fetch file names and requester names
      ...
      const formatted = requestsData.map(req => ({
        ...req,
        file_name: filesData?.find(f => f.id === req.file_id)?.name || 'Arquivo não encontrado',
        requester_name: profilesData?.find(p => p.id === req.requested_by)
          ? `${profilesData.find(p => p.id === req.requested_by)?.first_name} ${profilesData.find(p => p.id === req.requested_by)?.last_name}`
      ...
      <div className="p-4 border-b border-gray-100 bg-blue-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-blue-800">Solicitações de Arquivos (Renomear/Excluir)</h3>
